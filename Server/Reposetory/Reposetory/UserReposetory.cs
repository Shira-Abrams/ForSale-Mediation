...
namespace Reposetory.Reposetory
{
    public class UserReposetory:IReposetory<User>
    {
        ...
        {
            return await _icontext.Users.Include(x => x.ListOfSearching).Include(x => x.PropertyList).ToListAsync();
        }

        public async Task<User> GetByIdAsync(int id)
        {
          return await   _icontext.Users.Include(x=>x.ListOfSearching).Include(x=>x.PropertyList).FirstOrDefaultAsync(x=>x.Id==id);
